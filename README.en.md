...
The top-level `metadata` also stores how difficulty is computed (`k`, method priority, the staged labeling rule, and tie handling).

Difficulty labels are assigned with `k=10` using the following staged rule:

1. Label a query `easy` if `rank_by_mora_editdistance` still achieves recall@10=1 after pessimistically resolving same-score ties by placing negatives before positives
2. Otherwise label it `medium` if `rank_by_vowel_consonant_editdistance(vowel_ratio=0.8)` still achieves recall@10=1 under the same pessimistic tie handling
3. Label it `hard` if neither method stably achieves recall@10=1

Use `scripts/update_subset_labels.py` to regenerate these labels and refresh `recall_by_subset` in the checked-in evaluation result files.
