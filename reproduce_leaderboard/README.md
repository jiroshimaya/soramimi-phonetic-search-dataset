...
軽量な試行錯誤用として、既存の `results/*.json` の**先頭10件だけを再集計**した [`leaderboard.small.md`](./leaderboard.small.md) も用意しています。こちらは開発・研究時の補助ビューで、トップレベル README からはリンクしていません。
...
small dataset を使って試した結果は、必要なら `results_small/` のような別ディレクトリに保存すると整理しやすいです。`leaderboard.small.md` 自体は、リポジトリにある既存の `results/*.json` から再計算しています。
